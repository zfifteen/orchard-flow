...
int main(int argc, char** argv) {
  try {
    const std::string config_path =
        argc > 1 ? argv[1] : "benchmarks/channel_couette_128.cfg";
    const solver::ChannelFlowConfig config = solver::load_channel_flow_config(config_path);
    ...
    std::cout << "profile_relative_l2_error: " << result.validation.relative_l2_error << '\n';
    ...
    std::cerr << "solver_channel failed: " << exception.what() << '\n';
    return 1;
  }
}
